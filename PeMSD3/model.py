...
def STJGCN(x, t, A_fw, A_bw, Q, T, d, K, delta_adt, is_training):
    ...
    assert K == 2
    # STJGC1  output: [1, 3, 5, 7, 9, 11]
    x_seq = [x[:, ::K], x[:, 1::K]]  # [t - 1, t]
    U_seq = [U[:, ::K], U[:, 1::K]]
    A_fw_seq = [A_fw[1], A_fw[0]]
    A_bw_seq = [A_bw[1], A_bw[0]]
    z1, U = STJGC(x_seq, A_fw_seq, A_bw_seq, U_seq, B, delta_adt, is_training)
    z2 = tf.add(x[:, 1::K], z1) # residual connection
    # STJGC2  output: [3, 7, 11]
    x_seq = [z2[:, ::K], z2[:, 1::K]]  
    U_seq = [U[:, ::K], U[:, 1::K]]
    A_fw_seq = [A_fw[2], A_fw[0]] # time difference: 2
    A_bw_seq = [A_bw[2], A_bw[0]]
    z2, U = STJGC(x_seq, A_fw_seq, A_bw_seq, U_seq, B, delta_adt, is_training)
    z3 = tf.add(z1[:, 1::K], z2)    
    # STJGC3  output: [7, 11]
    x_seq = [z3[:, :-1:1], z3[:, 1::1]]  
    U_seq = [U[:, :-1:1], U[:, 1::1]]
    A_fw_seq = [A_fw[4], A_fw[0]] 
    A_bw_seq = [A_bw[4], A_bw[0]]
    z3, U = STJGC(x_seq, A_fw_seq, A_bw_seq, U_seq, B, delta_adt, is_training)
    z4 = tf.add(z2[:, 1::1], z3)
    # STJGC4  output: [11]
    x_seq = [z4[:, :-1:1], z4[:, 1::1]]  
    U_seq = [U[:, :-1:1], U[:, 1::1]]
    A_fw_seq = [A_fw[4], A_fw[0]] 
    A_bw_seq = [A_bw[4], A_bw[0]]
    z4, _ = STJGC(x_seq, A_fw_seq, A_bw_seq, U_seq, B, delta_adt, is_training)
    # multi-range attention
    z = tf.concat((z1[:, -1:], z2[:, -1:], z3[:, -1:], z4[:, -1:]), axis = 1)
